...
from load_images import RAW_DIR, build_dicom_lookup, find_images, find_labels_csv, load_image
...
def train():
    ...
    train_loader = DataLoader(
        ImageDataset(train_frame, path_col, label_col, transform=transform),
        batch_size=16,
        shuffle=True,
        num_workers=0,
    )
    val_loader = DataLoader(
        ImageDataset(val_frame, path_col, label_col, transform=transform),
        batch_size=16,
        shuffle=False,
        num_workers=0,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default=str(ROOT / 'resnet18_cancer.pth'), help='Trained model path')
    ...
    if args.train:
        ...
        import csv
        out_path = ROOT / 'datasets' / 'predictions.csv'
    ...
    if args.aggregate or args.all:
        ...
        import pandas as pd
        preds = pd.read_csv(ROOT / 'datasets' / 'predictions.csv')
        ...
        out_agg = ROOT / 'datasets' / 'predictions_by_uuid.csv'
        agg.to_csv(out_agg)
